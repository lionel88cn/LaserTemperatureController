...
uint16_t setTempearture()
{
	...
	if (process_sensor)
	{
		...
		posArray[0]=MyLinRots[0].p_Data->Position;
		curPos=(posArray[0]+posArray[1]+posArray[2]+posArray[3]+posArray[4])/5;
		if(curPos>118)
		{ 
			pos=4;
			scaledTmp=scaledTmp+5;
		}
		else 
		{
			if(curPos>70)
		...
		if(scaledTmp>9999) scaledTmp=9999;
		if(scaledTmp<0) scaledTmp=0; 
	}
	else
	{
		posArray[0]=0;
